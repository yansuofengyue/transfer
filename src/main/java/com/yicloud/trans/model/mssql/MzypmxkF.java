package com.yicloud.trans.model.mssql;

import java.io.Serializable;
import java.math.BigDecimal;
import java.util.Date;

import com.baomidou.mybatisplus.annotation.TableId;
import lombok.Data;

/**
 * mzypmxk_f
 * @author 
 */
@Data
public class MzypmxkF implements Serializable {
    @TableId
    private Long id;

    private Integer zyh;

    private Date rq;

    private Date sfrq;

    private Date fyrq;

    private String gfbz;

    private String kdksh;

    private String ysh;

    private Integer cflx;

    private String cfh;

    private Integer ts;

    private BigDecimal je;

    private String lrgh;

    private String fygh;

    private String tzgh;

    private BigDecimal fjf;

    private String xm;

    private String rbbz;

    private Long mzyskdkId;

    private BigDecimal zfje;

    private Long sfbhId;

    private Integer cfly;

    private String bzTy;

    private Integer fyyf;

    private Integer fpxh;

    private BigDecimal xyje;

    private BigDecimal zcyje;

    private BigDecimal cyje;

    private BigDecimal zfXyje;

    private BigDecimal zfZcyje;

    private BigDecimal zfCyje;

    private Integer sfxmId;

    private Integer rbBzxh;


    private Integer bybz;

    private Integer window;

    private Integer psbz;

    private String psczgh;

    private Date pstssj;

    private Date pskssj;

    private Integer psjg;

    private String pssm;

    private Integer printBz;

    private BigDecimal clje;

    private BigDecimal zfClje;

    private String printLsh;

    private Date printRq;

    private Integer xtid;

    private String bjgbBz;

    private String lbh;

    private String gfma;

    private String gfsf;

    private String wy;

    private String cyfl;

    private Byte jybz;

    private String lyzh;

    private String bqdyBz;

    private String bz;

    private Byte mtbs;

    private String pfry;

    private String cfdyBz;

    private Long ycfid;

    private String shBz;

    private String klj;

    private Date dfdyrq;

    private Date rqOld;

    private String jyh;

    private String wpcfbz;

    private String wpcfbh;

    private String wjScbz;

    private Date wjScrq;

    private String pslsh;

    private String gfxggh;

    private Date gfxgrq;

    private String gfxgsm;

    private String pfryxm;

    private String fyxm;

    private String tzxm;

    private String tzxm2;

    private Date shrq;

    private String chdybz;

    private String wybz;

    private String ypns;

    private String ysBz;

    private BigDecimal yhje;

//    private String zzdbz;

    private static final long serialVersionUID = 1L;
}